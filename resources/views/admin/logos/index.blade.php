...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        ...
        <div class="mt-4 md:mt-0">
            <a href="{{ route('admin.content.logos.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        ...
        @if($currentLogo)
            <div class="flex flex-col md:flex-row items-center">
                <div class="bg-white p-4 rounded-lg border flex items-center justify-center" style="max-width: 300px; height: 150px;">
                    <img src="{{ $currentLogo->url }}" alt="Current Logo" class="max-h-full max-w-full object-contain">
                </div>
                <div class="mt-4 md:mt-0 md:ml-6 flex flex-col">
                    <p class="text-sm text-gray-500 mb-2">Diupload: {{ $currentLogo->created_at->format('d M Y, H:i') }}</p>
                    <p class="text-sm text-gray-500 mb-4">Nama file: {{ $currentLogo->path }}</p>
                </div>
            </div>
        @else
