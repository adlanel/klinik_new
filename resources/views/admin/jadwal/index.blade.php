...
<div class="space-y-3">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2">
            ...
            <div class="flex gap-2">
                <div class="bg-blue-50 p-2 rounded text-center">
                    <div class="text-lg font-bold text-blue-600">{{ $totalSesi }}</div>
                    <div class="text-xs text-blue-500">Sesi</div>
                </div>
                <div class="bg-green-50 p-2 rounded text-center">
                    <div class="text-lg font-bold text-green-600">{{ $totalPasien }}</div>
                    <div class="text-xs text-green-500">Pasien</div>
                </div>
                <div class="bg-purple-50 p-2 rounded text-center">
                    <div class="text-lg font-bold text-purple-600">{{ $totalTerapis }}</div>
    ...
    <div class="bg-white rounded-lg shadow-md p-3">
        ...
        <div class="mb-3">
            <div class="flex justify-between items-center mb-2">
                <h3 class="text-base font-semibold text-gray-800">
                    {{ $carbonDate->locale('id')->translatedFormat('F Y') }}
                </h3>
                <div class="flex gap-1">
                    <a href="{{ route('admin.jadwal.index', ['tanggal' => $carbonDate->copy()->subMonth()->format('Y-m-d')]) }}" 
                       class="p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded text-sm">
                        <i class="fas fa-chevron-left"></i>
                    </a>
                    <a href="{{ route('admin.jadwal.index', ['tanggal' => Carbon\Carbon::today()->format('Y-m-d')]) }}" 
                       class="px-2 py-1.5 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200">
                        Bulan Ini
                    </a>
                    <a href="{{ route('admin.jadwal.index', ['tanggal' => $carbonDate->copy()->addMonth()->format('Y-m-d')]) }}" 
                       class="p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded text-sm">
        ...
        <div class="border-t pt-3">
            <form method="GET" action="{{ route('admin.jadwal.index') }}" class="flex gap-2">
                ...
                @if($search)
                    <a href="{{ route('admin.jadwal.index', ['tanggal' => $tanggal]) }}" 
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-2 bg-gray-50 border-b">
            <h3 class="text-base font-semibold text-gray-800">
                Jadwal 30 Hari - {{ $monthDates[0]['carbonDate']->locale('id')->translatedFormat('d F') }} - {{ $monthDates[29]['carbonDate']->locale('id')->translatedFormat('d F Y') }}
            </h3>
        </div>
        ...
        <div class="overflow-x-auto relative" id="main-scroll">
            ...
            <div class="flex bg-gray-100 border-b sticky top-0 z-10" style="min-width: max-content;">
                ...
                @foreach($monthDates as $dateInfo)
                    ...
                    @endphp
                    <div class="w-64 flex-shrink-0 p-2 border-r text-center {{ $dateInfo['isToday'] ? 'bg-blue-100 text-blue-700' : ($dateInfo['isSelected'] ? 'bg-blue-200 text-blue-800' : '') }}">
                        <div class="text-xs font-medium whitespace-nowrap">
                            {{ $indonesianDay }} {{ $dateInfo['dayNumber'] }}
                        </div>
                    </div>
                @endforeach
            </div>
                        ...
                        <div class="w-64 flex-shrink-0 p-1 border-r min-h-8 {{ $dateInfo['isToday'] ? 'bg-blue-25' : '' }}">
                            ...
                            @else
                                <div class="text-gray-300 text-xs italic h-8 flex items-center justify-center hover:bg-gray-100 hover:text-gray-600 transition-all cursor-pointer border border-transparent hover:border-gray-300 rounded" 
                                     onclick="addSession('{{ $dateInfo['date'] }}', '{{ $jam }}')">
                                    + Tambah Sesi
                                </div>
                            @endif
                        </div>
