...
<div class="container px-6 py-8 mx-auto">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.content.fasilitas.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition">
    ...
    <div class="mt-8">
        ...
        <div class="bg-white p-6 rounded-md shadow">
            <form action="{{ route('admin.content.fasilitas.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            
                <div class="mb-6">
                    <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Judul Fasilitas</label>
                    <input type="text" name="title" id="title" value="{{ old('title', $fasilitas->title) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200" required>
                    @error('title')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="short_description" id="short_description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200">{{ old('short_description', $fasilitas->short_description) }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="10" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200">{{ old('description', $fasilitas->description) }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-4">
                        <img src="{{ $fasilitas->image_url }}" alt="Current Image" class="h-40 object-contain border rounded">
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="status" id="status" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200">
                        <option value="active" {{ old('status', $fasilitas->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $fasilitas->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
