...
@section('title', 'Tambah Data Pasien Baru')
    ...
    <form action="{{ route('admin.patients.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="nama_anak" id="nama_anak" required value="{{ old('nama_anak') }}" 
            ...
            <div>
                ...
                <select name="id_cabang" id="id_cabang" required 
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ old('id_cabang') == $branch->id ? 'selected' : '' }}>{{ $branch->nama_cabang }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="3" 
                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('alamat') }}</textarea>
        </div>
