    ...
    <form action="{{ route('admin.patients.update', $patient->id_pasien) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="jenis_terapi" id="jenis_terapi" 
                    ...
                    <option value="Terapi Wicara" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Terapi Wicara' ? 'selected' : '' }}>Terapi Wicara</option>
                    <option value="Terapi Okupasi" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Terapi Okupasi' ? 'selected' : '' }}>Terapi Okupasi</option>
                    <option value="Fisioterapi Anak" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Fisioterapi Anak' ? 'selected' : '' }}>Fisioterapi Anak</option>
                    <option value="Terapi Perilaku" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Terapi Perilaku' ? 'selected' : '' }}>Terapi Perilaku</option>
                    <option value="Terapi Edukasi" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Terapi Edukasi' ? 'selected' : '' }}>Terapi Edukasi</option>
                    <option value="Terapi Sensori Integrasi" {{ old('jenis_terapi', $patient->jenis_terapi) == 'Terapi Sensori Integrasi' ? 'selected' : '' }}>Terapi Sensori Integrasi</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="terakhir_konsultasi" id="terakhir_konsultasi" value="{{ old('terakhir_konsultasi', $patient->terakhir_konsultasi) }}" 
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="hasil_follow_up" id="hasil_follow_up" rows="3" 
                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('hasil_follow_up', $patient->hasil_follow_up) }}</textarea>
        </div>
