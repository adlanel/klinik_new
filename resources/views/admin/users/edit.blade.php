...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="max-w-3xl">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" class="w-full rounded-md @error('role') border-red-300 @else border-gray-300 @enderror shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                        ...
                        <option value="terapis" {{ (old('role', $user->role) == 'terapis') ? 'selected' : '' }}>Terapis</option>
                        <option value="kepala_terapis" {{ (old('role', $user->role) == 'kepala_terapis') ? 'selected' : '' }}>Kepala Terapis</option>
                    </select>
                    @error('role')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="bidang" id="bidang" value="{{ old('bidang', $user->bidang) }}" 
        ...
        <div class="flex justify-between items-center pt-4 border-t border-gray-200">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('admin.users.change-password', $user->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-6 rounded">
